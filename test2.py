...
def scrap_lazada(search_item, total_of_result, queue):
    ...
    for product in oJson:
        results.append(Lazada(product['name'], "$"+product['offers']
                              ['price'], "Unavailable", product['url'], product['image']))
    ...
    for i in range(len(results)):
        ...
        num_of_ratings = [
            i for i in ratings_count[0].text if i.isdigit() == True]
        # checking if the product has reviews
        num_rating = join(num_of_ratings)
        if num_rating >= 1:
            firebase.patch("/Products/{}".format(search_item),
                           {"url": product['url']})
